...
> decks, EQ, faders, beatgrids, or beatmatching here.
...
3. **One audio graph, one master.** All audio (both decks during a crossfade) sums
   into the **master gain → analyser → destination** in [audio-engine.js](../src/renderer/audio-engine.js).
   Visualizers read that master analyser, so they always see the live signal. Never
   give a feature its own `AudioContext` or its own path to `destination`.
...
5. **The engine knows nothing about the UI or visualizers.** It exposes only the
   analyser nodes and playback controls (`load`, `play`, `crossfadeTo`, …). Keep
   transport/queue/history wiring in `app.js`; keep audio in `audio-engine.js`.
...
src/
  ...
  renderer/             # sandboxed page, ES modules, no framework
    app.js              #   WIRING hub: transport, queue, auto-crossfade, history, suggestions
    ...
    mini-spectrum.js    #   LCD spectrum analyzer in the player console (a manager overlay)
    viz/
      ...
      waveform.js spectrograph.js spectrogram-analyzer.js stereograph.js particles.js
      blackwhite.js reaction.js sacred.js esoteric.js
```
...
2. Register it in [app.js](../src/renderer/app.js): import it and add the class to the
   `[Waveform, Spectrograph, SpectrogramAnalyzer, Stereograph, Particles, BlackWhite,
   Reaction, SacredGeometry, Esoteric]` list. That's all — the "Off" toggle and the
   buttons are generated from the registry.
...
- Concise, expression-style helpers and one-liners where the existing code uses them;
  small pure functions over classes unless modeling stateful objects (engine,
...
- Naming follows the codebase: the two crossfade decks are `'A'`/`'B'`, with
  `which`/`other(which)` and `primary` (the deck the transport follows).
- Clamp and guard audio inputs (`clamp01`, check `Number.isFinite` on
...
- CSS lives in [style.css](../src/renderer/style.css) using the existing CSS variables
  (`--bg`, `--accent`, `--accent-2`, `--panel`, `--lcd`, `--vis-*`, …). Theme is dark
  and neon with quiet CRT scanlines — an early-2000s player vibe. **Reuse variables;
  never hard-code a colour** — a hard-coded colour won't follow the skins.
- **Skins** are palettes that override those variables on `body[data-skin="…"]`
  (top of `style.css`); the `🎨 Skin` switcher in `app.js` cycles them and persists the
  choice. Any new colour you introduce must be a variable defined in *every* skin block,
  or it will look wrong in three of the four skins. The mini spectrum analyzer reads its
  `--vis-*` colours from computed style, so re-skin via `refreshColors()` if you touch it.
...
- **Crossfader gates audio.** Each deck's crossfader gain starts at 0; the equal-power
  crossfader decides what reaches master. During a `load`, the primary deck is brought
  to 1 and the other to 0. A deck can be playing yet silent because the crossfader is
  on the other side. Keep this in mind before concluding "a deck doesn't output."
...
- **The auto-transition is a *pure volume* crossfade** — a Spotify-style segue, no
  tempo-matching. `crossfadeTo(url, { duration })` is the only mixing the player does.
...
config (`webPreferences`, CSP, preload surface), or the primary-deck event forwarding.
Open these as small PRs with a clear description of the invariant you believe you're
preserving.
